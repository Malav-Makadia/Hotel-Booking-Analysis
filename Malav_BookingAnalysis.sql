...
#1
SELECT city, SUM((room_price * number_of_nights)) as revenue FROM bookings Group by City;

#2
SELECT customer_id,COUNT(*) AS total_bookings FROM bookings 
WHERE booking_date >= date('now', '-1 year')
GROUP BY customer_id HAVING total_bookings > 3;

#3
SELECT
    property_id,
    Round((SUM(CASE WHEN booking_status = 'cancelled' THEN 1 ELSE 0 END) * 100.0 / COUNT(*)),2) AS cancellation_rate
From bookings Group by property_id; 

#4
With city_monthly_revenue AS(SELECT 
	city,
    strftime('%Y-%m', booking_date) AS month,
    SUM(revenue) as revenue,
    RANK() OVER (PARTITION BY city ORDER BY revenue DESC) AS rnk
FROM bookings WHERE booking_status='confirmed' GRoup by city,month) 
SELECT 
  city, month, revenue
FROM city_monthly_revenue WHERE rnk = 1;

#5
SELECT property_id, (occupied_rooms*100)/total_rooms as occupany_rate
from properties GROUP BY property_id HAVING occupany_rate> 90.0;